...
- **User in responses:** For the **authenticated user only** (login, me, signup, refresh, confirm-email-change, update-profile), use `PublicUser` (id, shortId, email, username, displayName) via `userToJson`. Email/username are PII and must not be returned for _other_ users. For any response that describes another user (e.g. bucket admins list/detail), use `PublicUserSummary` (id, shortId, displayName) via `userToPublicSummary` from `apps/api/src/lib/userToJson.ts`.
