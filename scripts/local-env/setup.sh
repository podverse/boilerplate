...
[ -f "$WEB_SIDECAR_INFRA_ENV" ] || cp "$ENV_TEMPLATES/web-sidecar.env.example" "$WEB_SIDECAR_INFRA_ENV"
...
[ -f "$MANAGEMENT_WEB_SIDECAR_INFRA_ENV" ] || cp "$ENV_TEMPLATES/management-web-sidecar.env.example" "$MANAGEMENT_WEB_SIDECAR_INFRA_ENV"
...
for f in "$MANAGEMENT_API_APP_ENV" "$MANAGEMENT_API_INFRA_ENV"; do
  ...
  upsert_var "$f" "DB_READ_PASSWORD" "$DB_READ_PASSWORD"
  upsert_var "$f" "DB_READ_WRITE_PASSWORD" "$DB_READ_WRITE_PASSWORD"
  upsert_var "$f" "MANAGEMENT_DB_PASSWORD" "$DB_READ_WRITE_PASSWORD"
  upsert_var "$f" "VALKEY_PASSWORD" "$VALKEY_PASSWORD"
done
