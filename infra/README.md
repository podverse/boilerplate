...
Directory layout for local and containerized run of the Metaboost stack (aligned with podverse monorepo conventions). No k8s in this repo.
...
- **docker/local/** – Dockerfiles and docker-compose for api, web, sidecar, postgres, and valkey. Combined stack (from repo root): `docker compose -f infra/docker/local/docker-compose.yml --project-directory . up --build`. Shared network `metaboost_local_network` is created on first up. Copy `infra/config/env-templates/db.env.example` and `valkey.env.example` to `infra/config/local/` for DB and Valkey env when running the API in Docker.
