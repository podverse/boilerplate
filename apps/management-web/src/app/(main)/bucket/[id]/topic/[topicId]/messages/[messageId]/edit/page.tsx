...
import { bucketViewRoute, topicMessagesRoute } from '../../../../../../../../../lib/routes';

const requestOptions = { cache: 'no-store' as RequestCache } as const;

async function fetchBucket(id: string) {
  ...
  const data = res.data as { bucket?: { id: string; parentBucketId: string | null } };
...
export default async function TopicMessageEditPage({
  ...
  return (
    <BucketMessageEditClient
      ...
      messagesRoute={topicMessagesRoute(parentId, topicId)}
    />
  );
}
