...
test.describe('This suite verifies the management users-new-page for the admin with buckets read, no users CRUD user.', () => {
  test('When an admin without usersCrud create (e.g. admin-with-bucket-admins) opens the users-new-page, they are redirected to the dashboard.', async ({
    page,
  }, testInfo) => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The admin without users create is redirected to the dashboard when opening the users-new-page.'
    );
  });
});
