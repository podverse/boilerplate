...
test.describe('This suite verifies the management admin-detail-page for the admin with buckets read, no admins CRUD user.', () => {
  test('When an admin without adminsCrud (e.g. admin-with-bucket-admins) opens the admin-detail-page, they are redirected to the dashboard.', async ({
    page,
  }, testInfo) => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The admin without adminsCrud is redirected to the dashboard when opening the admin-detail-page.'
    );
  });
});
