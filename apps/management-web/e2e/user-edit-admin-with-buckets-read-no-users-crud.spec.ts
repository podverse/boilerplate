...
test.describe('This suite verifies the management user-edit-page for the admin with buckets read, no users CRUD user.', () => {
  test('When an admin without usersCrud (e.g. admin-with-bucket-admins) opens the user-edit-page, they are redirected to the dashboard.', async ({
    page,
  }, testInfo) => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The admin without usersCrud is redirected to the dashboard when opening the user-edit-page.'
    );
  });
});
