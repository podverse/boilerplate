...
/**
 * Bucket-edit redirects to bucket-settings; settings layout gates by bucketsCrud read → redirect
 * dashboard. Actor matrix: unauthenticated → login; super-admin and admin with buckets read →
 * redirect to settings and see form; limited-admin (no buckets read) → redirect to dashboard.
 */

test.describe('This suite verifies the management bucket-edit-page: unauthenticated redirect, invalid bucket id→not found, permitted role redirected to settings with form, list→edit, Cancel→bucket-view, Save→bucket-view and persistence.', () => {
  ...
  test('When a limited-admin (no buckets read) opens the bucket-edit-page, they are redirected to the dashboard.', async ({
    page,
  }, testInfo) => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The limited-admin is redirected to the dashboard when opening the bucket-edit-page.'
    );
  });

  test('When an admin with buckets read opens the bucket-edit-page, they are redirected to the bucket-settings-page and see the bucket-edit-form.', async ({
    page,
  }, testInfo) => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The admin with buckets read sees the bucket-edit-form after redirect from bucket-edit.'
    );
  });
});
