...
/**
 * Permission: bucketsCrud create required; else redirect /buckets. Actor matrix: unauthenticated →
 * login; super-admin → form; limited-admin (no buckets read) → redirect /buckets then dashboard;
 * admin with buckets read but no create → redirect /buckets.
 */

test.describe('This suite verifies the management buckets-new page: unauthenticated redirect, permitted role sees form, validation, create success, Cancel→buckets-list, and flow from buckets-list to new-bucket form.', () => {
  ...
  test('When a limited-admin (no buckets create permission) opens the buckets-new route, they are redirected to the buckets-list.', async ({
    page,
  }, testInfo) => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The limited-admin is redirected to the buckets-list when visiting the buckets-new route.'
    );
  });
