...
test.describe('This suite verifies the management events-page for the admin-without-bucket-admins-crud user.', () => {
  test('When the admin with event-visibility all_admins opens the events-page, only admin actor events are shown.', async ({
    page,
  }, testInfo) => {
    setE2EUserContext(testInfo, 'admin-without-bucket-admins-crud (events all_admins)');
