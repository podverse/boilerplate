...
export default defineConfig({
  ...
  webServer: [
    ...
    {
      command:
        'PORT=4012 RUNTIME_CONFIG_URL=http://localhost:4011 NEXT_PUBLIC_API_URL=http://localhost:4010 NEXT_PUBLIC_AUTH_MODE=admin_only_username npm run build -w @boilerplate/web && NODE_OPTIONS="--disable-warning=DEP0060" PORT=4012 RUNTIME_CONFIG_URL=http://localhost:4011 NEXT_PUBLIC_API_URL=http://localhost:4010 NEXT_PUBLIC_AUTH_MODE=admin_only_username npm run start -w @boilerplate/web',
      port: 4012,
      cwd: '../..',
      reuseExistingServer: false,
      timeout: 420_000,
