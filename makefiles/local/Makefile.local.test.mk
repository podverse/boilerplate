...
# Drops and recreates the test DB each time so the schema always matches infra/k8s/base/stack/postgres-init/z_load_app_schema.sql.
...
test_db_init: test_postgres_up
	...
	@cat infra/k8s/base/stack/postgres-init/z_load_app_schema.sql | docker exec -i $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_DB_NAME)
...
test_db_init_management: test_db_init
	...
	@cat infra/k8s/base/stack/postgres-init/z_load_management_schema.sql | docker exec -i $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_MANAGEMENT_DB_NAME)
...
help_test:
	...
	@echo "  3. Drop and recreate $(TEST_DB_NAME), apply infra/k8s/base/stack/postgres-init/z_load_app_schema.sql, and create boilerplate_app_read/boilerplate_app_read_write users."
	@echo "  4. Drop and recreate $(TEST_MANAGEMENT_DB_NAME), apply infra/k8s/base/stack/postgres-init/z_load_management_schema.sql (for management-api tests)."
