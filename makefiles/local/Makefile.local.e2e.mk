...
.PHONY: e2e_deps e2e_seed e2e_seed_web e2e_seed_management_web e2e_test_api e2e_test e2e_test_web e2e_test_management_web e2e_test_web_home e2e_test_management_web_home e2e_test_home e2e_test_home_report e2e_test_web_report_spec e2e_test_management_web_report_spec e2e_test_report_scoped e2e_test_report e2e_teardown
...
# Run API gate decision first; if gate runs and succeeds, re-seed, then run Playwright for management-web only.
# Playwright auto-starts management-api/management-web on dedicated E2E ports (4110/4112)
# using production-like build/start commands.
e2e_test_management_web:
	@$(call e2e_run_api_gate)
	@$(MAKE) e2e_seed_management_web
	@npm run test:e2e -w apps/management-web

# Run API gate decision first; if gate runs and succeeds, re-seed web, then run only web home smoke test.
...
# Run API gate decision first; if gate runs and succeeds, re-seed management-web, then run only management home smoke test.
# Uses isolated E2E app ports (4110/4112); no dependency on default dev ports.
e2e_test_management_web_home:
	@$(call e2e_run_api_gate)
	@$(MAKE) e2e_seed_management_web
	@npm run test:e2e -w @boilerplate/management-web -- e2e/home.spec.ts

# Run API gate decision first; if gate runs and succeeds, re-seed both, then run only home smoke tests for both apps.
...
# Runs API gate decision first, re-seeds management-web, captures step screenshots, writes timestamped report, rotates to 10 runs.
