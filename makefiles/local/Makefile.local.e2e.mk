...
.PHONY: e2e_deps e2e_seed e2e_seed_web e2e_seed_management_web e2e_test_api e2e_test e2e_test_web e2e_test_management_web e2e_test_web_home e2e_test_management_web_home e2e_test_home e2e_test_home_report e2e_teardown
...
# then open both result pages in the default system browser.
e2e_test_home_report: e2e_test_api e2e_seed
	...
	E2E_STEP_SCREENSHOTS=true PLAYWRIGHT_HTML_OPEN=never PLAYWRIGHT_HTML_OUTPUT_DIR="$$WEB_REPORT_DIR" npm run test:e2e -w @boilerplate/web -- --reporter=html e2e/home.spec.ts; \
	E2E_STEP_SCREENSHOTS=true PLAYWRIGHT_HTML_OPEN=never PLAYWRIGHT_HTML_OUTPUT_DIR="$$MGMT_REPORT_DIR" npm run test:e2e -w @boilerplate/management-web -- --reporter=html e2e/home.spec.ts; \
	...
	if command -v open >/dev/null 2>&1; then \
		open "$$WEB_INDEX" || echo "Could not auto-open $$WEB_INDEX"; \
		open "$$MGMT_INDEX" || echo "Could not auto-open $$MGMT_INDEX"; \
	elif command -v xdg-open >/dev/null 2>&1; then \
		xdg-open "$$WEB_INDEX" >/dev/null 2>&1 || echo "Could not auto-open $$WEB_INDEX"; \
		xdg-open "$$MGMT_INDEX" >/dev/null 2>&1 || echo "Could not auto-open $$MGMT_INDEX"; \
	else \
