...
2. **Custom roles** live in schema tables (`bucket_role`, `management_admin_role`). Adding columns or changing CRUD semantics may require:
   - Migrations and init SQL updates together (keep both in sync):
     - Main DB: `infra/database/migrations/*.sql` + `infra/k8s/base/stack/postgres-init/z_load_app_schema.sql` (generated)
     - Management DB: `infra/management-database/migrations/*.sql` + `infra/k8s/base/stack/postgres-init/z_load_management_schema.sql` (generated)
...
- DB (main): `infra/k8s/base/stack/postgres-init/z_load_app_schema.sql`, `infra/database/migrations/*.sql`
- DB (management): `infra/k8s/base/stack/postgres-init/z_load_management_schema.sql`, `infra/management-database/migrations/*.sql`
