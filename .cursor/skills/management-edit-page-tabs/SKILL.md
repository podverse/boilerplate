...
- Use `Tabs` from `@boilerplate/ui` with `LinkComponent={Link}`, `activeHref` set to the **canonical** tab href (see [tab-active-state-url-match](.cursor/skills/tab-active-state-url-match/SKILL.md)), and `exactMatch`.
