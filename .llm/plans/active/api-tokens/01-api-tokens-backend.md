...
- `infra/k8s/base/stack/postgres-init/0003_app_schema.sql` / `0005_management_schema.sql.frag` – combined init SQL (from migrations via `combine-migrations.sh`)
...
1. Create a **new migration file** (do not edit the combined file directly):  
   ...
   The combined file `infra/k8s/base/stack/postgres-init/0003_app_schema.sql` is generated from migrations;  
...
2. Follow existing test patterns (supertest, app factory, test DB). Test DB is initialized
   with `infra/k8s/base/stack/postgres-init/0003_app_schema.sql` (see `apps/api/src/test/setup.ts` and
