...
existing values across runs. **Postgres env is a single merged file**: `infra/config/local/db.env` holds all classification **`db`** workload keys (single flat **`vars`** block in **`base.yaml`**, including **`DB_MANAGEMENT_SUPERUSER_*`** with **`override_file: db_management_superuser`**), generated by `merge-env --profile local_docker --workload db`. Docker Compose uses `--env-file infra/config/local/db.env` for the `postgres` service and for compose variable interpolation so `DB_USER` / `DB_PASSWORD` / `DB_APP_NAME` can bridge to the official image’s `POSTGRES_*` vars. API and management-api get different JWT secrets (`API_JWT_SECRET` and
...
| auth.env                    | **`AUTH_MODE`** (workload **`auth`**): API, management-api, and web (**`NEXT_PUBLIC_AUTH_MODE`** via [`scripts/local-env/setup.sh`](../../scripts/local-env/setup.sh)). Sensible default in example (admin_only_username).                                                                                                                                                                                                                        |
